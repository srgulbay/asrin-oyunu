...
async function startTournament() {
    ...
    try {
        if (!pool) {
            // Veritabanı yoksa örnek soruları kullan
            console.warn("UYARI: Veritabanı bağlantısı yok, örnek sorular kullanılıyor.");
            gameQuestions = [
            ...
            const result = await pool.query('SELECT id, question_text, options, correct_answer FROM questions ORDER BY RANDOM() LIMIT 5');
             if (result.rows.length === 0) {
                 ...
                 // Hata vermek yerine örnek soruları kullanabiliriz
                  gameQuestions = [
                      { id: 1, question_text: '1+1 Kaç Yapar?', options: ['1', '2', '3', '4'], correct_answer: '2' },
                      { id: 2, question_text: 'Türkiye\'nin başkenti?', options: ['İstanbul', 'İzmir', 'Ankara', 'Bursa'], correct_answer: 'Ankara' },
                      { id: 3, question_text: 'React bir ...?', options: ['Framework', 'Kütüphane', 'Dil', 'Veritabanı'], correct_answer: 'Kütüphane' }
...
function sendNextQuestion() { clearTimeout(questionTimer); currentQuestionIndex++; if (currentQuestionIndex >= gameQuestions.length) { endTournament(); return; } const question = gameQuestions[currentQuestionIndex]; const questionData = { index: currentQuestionIndex, total: gameQuestions.length, text: question.question_text, options: question.options, timeLimit: QUESTION_TIME_LIMIT }; console.log(`Soru <span class="math-inline">\{currentQuestionIndex \+ 1\}/</span>{gameQuestions.length} gönderiliyor: ${question.question_text}`); questionStartTime = Date.now(); io.to(TOURNAMENT_ROOM).emit('new_question', questionData); questionTimer = setTimeout(() => { console.log(`Soru ${currentQuestionIndex + 1} için süre doldu.`); io.to(TOURNAMENT_ROOM).emit('question_timeout', { questionIndex: currentQuestionIndex }); sendNextQuestion(); }, QUESTION_TIME_LIMIT * 1000); }
function endTournament() { clearTimeout(questionTimer); console.log("Turnuva bitti!"); currentGameState = GAME_MODES.GAME_OVER; const results = getSortedPlayerList().map(({id, name, score}) => ({id, name, score})); io.to(TOURNAMENT_ROOM).emit('game_over', { results }); setTimeout(() => { console.log("Oyun durumu IDLE'a dönüyor."); currentGameState = GAME_MODES.IDLE; tournamentPlayers.clear(); gameQuestions = []; currentQuestionIndex = -1; io.to(TOURNAMENT_ROOM).emit('reset_game', { message: 'Oyun bitti. Yeni oyun bekleniyor.' }); }, 15000); }
...
io.on('connection', (socket) => {
  ...
  socket.on('join_tournament', (data) => {
    ...
    // Bu scope içinde GAME_MODES'a erişimi kontrol et (debug için)
    // console.log('join_tournament - GAME_MODES check:', typeof GAME_MODES);
    ...
    if (tournamentPlayers.has(socket.id)) { console.log(`${playerName} zaten listede.`); socket.join(TOURNAMENT_ROOM); return; }
    console.log(`Oyuncu <span class="math-inline">\{socket\.id\} \(</span>{playerName}) turnuvaya katılıyor.`);
    socket.join(TOURNAMENT_ROOM);
    tournamentPlayers.set(socket.id, { name: playerName, score: 0, combo: 0, isReady: false }); // Combo eklendi
  ...
  socket.on('player_ready', () => {
    // console.log('player_ready - GAME_MODES check:', typeof GAME_MODES);
    ...
    if (!player.isReady) {
         player.isReady = true; console.log(`Oyuncu <span class="math-inline">\{player\.name\} \(</span>{socket.id}) hazır.`);
         broadcastTournamentState();
  ...
  socket.on('submit_answer', (data) => {
    ...
    // console.log('submit_answer - GAME_MODES check:', typeof GAME_MODES);
    ...
    const player = tournamentPlayers.get(socket.id);
    // if (player.lastAnsweredIndex === currentQuestionIndex) return; // Zaten cevapladıysa tekrar işleme
  ...
  socket.on('disconnect', (reason) => {
    // console.log('disconnect - GAME_MODES check:', typeof GAME_MODES);
    console.log(`Ayrıldı: ${socket.id}. Sebep: ${reason}`);
    if (tournamentPlayers.has(socket.id)) {
         const player = tournamentPlayers.get(socket.id);
         const wasReady = player.isReady; tournamentPlayers.delete(socket.id);
         ...
         if (currentGameState === GAME_MODES.TOURNAMENT_RUNNING || currentGameState === GAME_MODES.WAITING_TOURNAMENT) {
              ...
              if (currentGameState === GAME_MODES.WAITING_TOURNAMENT && wasReady) {
                   const allPlayersArray = Array.from(tournamentPlayers.values());
                   // Oyuncu ayrıldıktan sonra kalanlar hazırsa oyunu başlatmayı dene
                   if (allPlayersArray.length >= 1 && allPlayersArray.every(p => p.isReady)) {
                        console.log("Hazır oyuncu ayrıldı, kalanlar hazır. Turnuva başlatılıyor...");
                        startTournament();
                   }
              }
              // Eğer oyun devam ediyorsa ve hiç oyuncu kalmadıysa oyunu bitir
              if (currentGameState === GAME_MODES.TOURNAMENT_RUNNING && tournamentPlayers.size < 1) {
                   console.log("Oyuncu kalmadı, turnuva bitiriliyor.");
                   endTournament();
               }
          }
     }
...
app.get('/', (req, res) => {
   ...
   // console.log('/ route - GAME_MODES check:', typeof GAME_MODES);
...
server.listen(PORT, () => {
  ...
  // FRONTEND_URL kontrolünü kaldırıldı, allowedOrigins logu yeterli
}); // server.listen sonu
