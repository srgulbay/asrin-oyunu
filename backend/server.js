...
const crypto = require('crypto'); // Benzersiz ID için crypto modülü
...
// FRONTEND_URL çevre değişkeninden okunacak (SONUNDA ; OLMAMALI!)
...
let currentQuestionAnswers = new Map(); // Map<socketId, {answer, timeMs, correct}>
...
function generateQuestionSummaryAnnouncements(qIndex) {
    if (qIndex >= gameQuestions.length) return; // Geçersiz index ise çık
...
io.on('connection', (socket) => {
  ...
  socket.on('submit_answer', (data) => {
    ...
    socket.emit('answer_result', { correct, score: player.score, pointsAwarded, combo: player.combo, comboBroken, questionIndex: currentQuestionIndex }); broadcastTournamentState();
  });
