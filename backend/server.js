...
function endTournament() { clearTimeout(questionTimer); console.log("Turnuva bitti!"); currentGameState = GAME_MODES.GAME_OVER; const results = Array.from(tournamentPlayers.entries()).map(([id, data]) => ({ id, name: data.name, score: data.score })).sort((a, b) => b.score - a.score); io.to(TOURNAMENT_ROOM).emit('game_over', { results }); setTimeout(() => { console.log("Oyun durumu IDLE'a dönüyor."); currentGameState = GAME_MODES.IDLE; tournamentPlayers.clear(); gameQuestions = []; currentQuestionIndex = -1; io.to(TOURNAMENT_ROOM).emit('reset_game', { message: 'Oyun bitti. Yeni oyun bekleniyor.' }); }, 15000); }
...
io.on('connection', (socket) => {
  ...
  socket.on('join_tournament', (data) => {
    ...
    tournamentPlayers.set(socket.id, { score: 0, name: playerName, isReady: false });
  ...
  socket.on('submit_answer', (data) => {
    ...
    if (correctAnswer && data.answer === correctAnswer) {
         player.score += 10; console.log(`Doğru! <span class="math-inline">\{player\.name\} \(</span>{socket.id}) skoru: ${player.score}`);
    ...
    if (tournamentPlayers.has(socket.id)) {
         ...
         if (currentGameState === GAME_MODES.TOURNAMENT_RUNNING || currentGameState === GAME_MODES.WAITING_TOURNAMENT) {
              ...
              if (currentGameState === GAME_MODES.WAITING_TOURNAMENT && wasReady) {
                   const allPlayersArray = Array.from(tournamentPlayers.values());
                   if (allPlayersArray.length >= 1 && allPlayersArray.every(p => p.isReady)) { console.log("Hazır oyuncu ayrıldı, kalanlar hazır. Turnuva başlatılıyor..."); startTournament(); }
              }
              if (currentGameState === GAME_MODES.TOURNAMENT_RUNNING && tournamentPlayers.size < 1) { console.log("Oyuncu kalmadı, turnuva bitiriliyor."); endTournament(); }
          }
     }
  });
});
...
server.listen(PORT, () => {
  ...
  // FRONTEND_URL kontrolünü kaldıralım, allowedOrigins logu yeterli
