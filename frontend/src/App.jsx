...
import { motion, AnimatePresence } from 'framer-motion'; // <<< EKSİK IMPORT EKLENDİ
// --- Component Imports ---
...
function App() {
  ...
  // Debug Mount/Unmount...
  useEffect(() => { console.log('%c[DEBUG] App component MOUNTED', 'color: green; font-weight: bold;'); return () => { console.log('%c[DEBUG] App component UNMOUNTING!', 'color: red; font-weight: bold;'); }; }, []);
  // Tema Modu...
  ...
  // Firebase Auth Durum...
  useEffect(() => { console.log("Firebase Auth dinleyicisi kuruluyor..."); useUserStore.setState({ isLoading: true }); const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => { console.log("onAuthStateChanged tetiklendi. Firebase User:", firebaseUser); zustandSetUser(firebaseUser); }); return () => { console.log("Firebase Auth dinleyicisi kaldırılıyor."); unsubscribe(); } }, [zustandSetUser]);
  // Socket Bağlantısı...
  useEffect(() => {
      ...
      // ... (socket.on dinleyicileri öncekiyle aynı) ...
      ...
      return () => {
        ...
        newSocket.off('connect', handleConnect); newSocket.off('connect_error', handleConnectError); newSocket.off('disconnect', handleDisconnect); newSocket.off('error_message', handleErrorMessage); newSocket.off('reset_game', handleResetGame); newSocket.off('initial_state', handleInitialState); newSocket.off('tournament_state_update', handleStateUpdate); newSocket.off('new_question', handleNewQuestion); newSocket.off('question_timeout', handleQuestionTimeout); newSocket.off('answer_result', handleAnswerResult); newSocket.off('game_over', handleGameOver); newSocket.off('waiting_update', handleWaitingUpdate); newSocket.off('announcer_message', handleAnnouncerMessage); if(questionTimerIntervalRef.current) clearInterval(questionTimerIntervalRef.current); newSocket.disconnect();
      };
  }, [SERVER_URL]); // Dependency array'i güncelledim, state'leri buraya eklemek genellikle gereksiz re-render/reconnect yapar
  ...
  const renderCurrentScreen = () => {
       ...
       if (zustandIsLoading || (!isConnected && !connectionMessage.includes('Bağlantı hatası'))) { return <Box sx={{ display: 'flex', justifyContent: 'center', padding: 5 }}><CircularProgress /><Typography sx={{ml: 2}}>Yükleniyor...</Typography></Box>; }
  ...
  console.log(`%c[DEBUG] App Render. State: ${gameState}, Connected: ${isConnected}, Players: ${players.length}`, 'color: blue;');
