...
import Button from '@mui/material/Button'; // Yükleme butonu için
import InstallMobileIcon from '@mui/icons-material/InstallMobile'; // İkon (opsiyonel)
// --- Component Imports ---
...
function App() {
  ...
  useEffect(() => {
    const handleBeforeInstallPrompt = (event) => {
      // Tarayıcının varsayılan mini-infobar'ını göstermesini engelle
      event.preventDefault();
      // Olayı daha sonra kullanmak üzere sakla
      console.log("PWA Yükleme istemi yakalandı!");
      setInstallPromptEvent(event);
      // Butonu göster (eğer uygulama zaten standalone modda çalışmıyorsa)
  ...
  useEffect(() => {
      // ... (Bu useEffect içindeki socket bağlantı kodları aynı kalacak) ...
  ...
  // --- YENİ: PWA Yükleme Butonu Tıklama Handler ---
  const handleInstallClick = useCallback(async () => {
    if (!installPromptEvent) {
      console.log("Yükleme istemi mevcut değil.");
      return;
    }
    ...
    // Seçime göre log yazdır
    console.log(`PWA Yükleme sonucu: ${outcome}`);
    // İstem sadece bir kez kullanılabilir, state'i temizle
  ...
  const renderCurrentScreen = () => { /* ... önceki kod ... */ if (!isConnected && connectionMessage.includes('Bağlantı hatası')) { return <Typography color="error" align="center" sx={{ mt: 2}}>{connectionMessage}</Typography>; } if (!isConnected) { return <Box sx={{ display: 'flex', justifyContent: 'center', padding: 5 }}><CircularProgress /></Box>; } if (gameState === GAME_STATES.IDLE || (gameState === GAME_STATES.WAITING_TOURNAMENT && !players.find(p=>p.id === socket?.id))) { return <JoinScreen playerName={playerName} setPlayerName={setPlayerName} handleJoinTournament={handleJoinTournament} isConnected={isConnected} waitingMessage={waitingMessage}/>; } if (gameState === GAME_STATES.WAITING_TOURNAMENT) { return <WaitingLobby players={players} handlePlayerReady={handlePlayerReady} isPlayerReady={isPlayerReady} waitingMessage={waitingMessage} currentSocketId={socket?.id}/>; } if (gameState === GAME_STATES.TOURNAMENT_RUNNING) { return <GameInterface currentQuestion={currentQuestion} timeRemaining={timeRemaining} handleAnswerSubmit={handleAnswerSubmit} lastAnswerResult={lastAnswerResult}/>; } if (gameState === GAME_STATES.GAME_OVER) { return <ResultsScreen gameResults={gameResults} waitingMessage={waitingMessage} currentSocketId={socket?.id}/>; } return <Box sx={{ display: 'flex', justifyContent: 'center', padding: 5 }}><CircularProgress /></Box>; };
    ...
    <ThemeProvider theme={theme}>
       ...
       <Container maxWidth="xl" sx={{ marginTop: 2, paddingBottom: 4 }}>
           ...
           <Grid container spacing={2} sx={{ marginTop: 2 }} alignItems="flex-start">
               <Grid xs={12} md={ showSidebars ? 3 : 0 } order={{ xs: 2, md: 1 }} sx={{ display: showSidebars ? 'block' : 'none' }}>
               ...
               <Grid xs={12} md={ showSidebars ? 3 : 0 } order={{ xs: 3, md: 3 }} sx={{ display: showSidebars ? 'block' : 'none' }}>
                     <Box sx={{ position: 'sticky', top: '20px', maxHeight: 'calc(100vh - 100px)', overflowY: 'auto' }}>
